...
def set_master_port(parser, args):
    ...
    if args.m_port is not None:
       M_PORT = args.m_port
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        res = sock.connect_ex((M_HOST, M_PORT))
        if res != 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m_port", dest="port", action="store", type=str, help="Master Search Node Host for search system" )
    parser.add_argument("-m_host", dest="m_host", action="store", type=str, help="Master Search Node Host for search system" )
    parser.add_argument("-port", dest="m_port", action="store",  type=int, help="Port for worker node")
